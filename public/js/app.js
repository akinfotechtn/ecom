// AK INFOTECH - FRONTEND APP (AVAILABILITY / IN STOCK / OUT OF STOCK ENHANCED)
  ...
  if (currentUser) {
    container.innerHTML = `
      <div class="user-avatar-btn" id="userProfileBtn" title="${escapeHtml(currentUser.displayName || currentUser.email)}">
        ...
        <span style="font-weight: 700; font-size: 0.8rem; color: var(--text-dark); max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${escapeHtml(currentUser.displayName?.split(' ')[0] || 'Account')}</span>
        <button id="googleLogoutBtn" style="background: none; border: none; font-size: 0.8rem; color: #ef4444; margin-left: 4px;" title="Logout">✕</button>
      </div>
    `;

    document.getElementById('googleLogoutBtn')?.addEventListener('click', async (e) => {
      e.stopPropagation();
...
// RENDER PRODUCTS CATALOG GRID WITH AVAILABILITY (IN STOCK / OUT OF STOCK)
